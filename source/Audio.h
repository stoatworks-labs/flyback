...
/**
	...
	through Resolume's FFT buffer. What Flyback needs is the same cut: the
	full-range level (Audio Drive raises the supply voltage with it) and the
	onset detector (Audio Fires turns each onset into a bang, a spark or a
	restrike), including the primed first frame that keeps it from going deaf
	on a clip trigger.
	...
	**Normalised against its own recent peak**, so the same Audio Drive means
	the same overvoltage on a quiet stem and a mastered track. The cost is that a
	long loud passage reads as "1" throughout, because relative to the last
	few seconds it is.
*/
namespace flyback::audio
{
...
public:
	...
	/// On by default, and it must stay on: see Update(). The harness turns it
	/// off for `--negative`, to show `--onset` catches a detector that has
	/// gone deaf.
