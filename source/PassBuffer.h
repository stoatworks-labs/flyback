...
/**
    ...
    **It owns its filtering and its wrap**, because this plugin's buffers want
    different answers: the emission and light buffers are read texel for
    texel and must not be filtered; the glow levels are read between texels,
    and past the frame's edge they must read ZERO -- clamping to the edge
    would duplicate the edge's light into the halo and break conservation.
    (Copied from millpond, where the third answer was a periodic surface.)
*/
...
public:
	enum class Wrap
	{
		...
		Border ///< light: past the edge is nothing, so a bilinear read there fades to zero and adds none
	};
